...
import { TeamRoleToggle } from "@/components/TeamRoleToggle";
import {
  Badge,
  Card,
  CardHeader,
...
import type { PricingUnit, QboSyncState } from "@/lib/types";
...
const QBO_MESSAGES: Record<string, { tone: "notice" | "error"; text: string }> = {
  ...
  owner_only: { tone: "error", text: "Only the owner can connect QuickBooks" },
};
    ...
    <AppShell session={session}>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <Card className="self-start">
          <CardHeader
            title="Team"
            meta="Crew can work the route. Only owners see money."
          />
          <ul>
            {team.map((member) => (
              <li
              ...
              >
                ...
                <span className="flex items-center gap-2">
                  <Badge tone={member.role === "owner" ? "green" : "quiet"}>
                    {member.role === "owner" ? "Owner" : "Crew"}
                  </Badge>
                  {member.id === session.userId ? null : (
                    <TeamRoleToggle userId={member.id} role={member.role} />
                  )}
                </span>
              </li>
            ))}
